...
static int ft6x06_get_vendor_from_bootloader(tinno_ts_data *ts, uint8_t *pfw_vendor, uint8_t *pfw_version)
{
	...
	*pfw_version = -1;//Force to update.
        ...
        //LINE<JIRA_ID><DATE20180310><Force to update>zenghaihui
        ftm_ft6x06_force_update = 1;
        pr_err("%s(%d): set ftm_ft6x06_force_update = 1 \n", __func__, __LINE__);  
    
	//CTP_DBG("bootloader version = 0x%x\n", version_id);
        pr_err("%s(%d): bootloader version = 0x%x \n", __func__, __LINE__, version_id);  
...
void ft6x06_tp_upgrade(const char * ftbin_buf, int buf_len)
{
    ...
    pr_err("%s(%d): vl_tp_ver = %x, ic_type_flag = %d \n", __func__, __LINE__, vl_tp_ver, ic_type_flag);  
	...
	if(ic_type_flag){
       ...
       pr_err("%s(%d): vl_bin_ver = %x \n", __func__, __LINE__, vl_bin_ver);  
       
    if(vl_tp_ver == 0x00)
    {
        vl_tp_ver = get_fw_version_ext(); 
        pr_err("%s(%d): read again vl_tp_ver = %x \n", __func__, __LINE__, vl_tp_ver);  
    }
    ...
    //LINE<JIRA_ID><DATE20180228><helitai fw 0x15 and dijing fw 0x17 only for PR1,don't upgrade>zenghaihui
    //LINE<JIRA_ID><DATE20180228><helitai PR2 fw 0xa0 and dijing PR2 fw 0x21>zenghaihui
    #if defined(TP_UPDATE_K400)
	if(((vl_bin_ver > vl_tp_ver) && (vl_tp_ver > 0x17)) || (ftm_ft6x06_force_update == 1))
    #else
	...
	else
	{
		//CTP_DBG("tp firmware needn't upgrade. \n");
            pr_err("%s(%d) tp firmware needn't upgrade. \n", __func__, __LINE__);  
	}
