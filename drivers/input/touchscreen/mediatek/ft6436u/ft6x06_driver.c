...
static irqreturn_t tpd_eint_interrupt_handler(int irq, void *desc)
{
    ...
    CTP_DBG("guomingyi#tpd_eint_interrupt_handler\n");
...
static  int update_firmware_thread(void *priv)
{
    ...
    work_lock=1;
		...
		if (NORMAL_BOOT == get_boot_mode()){
            ...
            CTP_DBG("update_firmware_thread panel_vendor_buff = %x ", panel_vendor_buff);   
            
			if(panel_vendor_buff == FTS_CTP_VENDOR_DIJING)
			{
				ft6x06_tp_upgrade(ftbin_k400_dj, sizeof(ftbin_k400_dj));
			}
                    else if(panel_vendor_buff == FTS_CTP_VENDOR_HOLITECH)
			{
                        ft6x06_tp_upgrade(ftbin_k400_hlt, sizeof(ftbin_k400_hlt));
			}
		}
		else{
 		  CTP_DBG("update_firmware_thread : Dont upgrade");   
...
static int tpd_irq_registration(void)
{
    ...
    CTP_DBG("guomingyi#tpd_irq_registration\n");
    ...
    CTP_DBG("guomingyi#tpd_irq_registration:irq:%d,ret:%d\n",touch_irq, ret);
	return 0;
}
...
static int  tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    pr_err(" ft6x06_driver.c TPD probe\n");
    
    if ( tpd_load_status )
    {
        pr_err(" ft6x06_driver.c Already probed a TP, needn't to probe any more!");
        return -1;
    }
    ...
    CTP_DBG("guomingyi#TPD enter tpd_probe ts=0x%p, TPD_RES_X=%d, TPD_RES_Y=%d, addr=0x%x\n", ts, TPD_RES_X, TPD_RES_Y, client->addr);
    /** client->addr = 0x38; */
    ...
    TPD_DMESG("guomingyi@Product version is %d,panel_vendor:%d\n", panel_version, panel_vendor);
    if ( panel_version < 0 || panel_vendor<0 || ret<0 )
    {
...
//END<touch panel><date20131028><tp auto update>yinhuiyong
    ...
    pr_err(" ft6x06_driver.c TPD probe done\n");
...
err_check_functionality_failed:
    pr_err("ft6x06_driver.c Touch Panel Device Probe FAIL\n");
    return -1;
}
...
void ft6x06_tpd_get_fw_version( char * fw_vendor_numb )
{
	u8 version_info = get_fw_version_ext();
        ...
        TPD_DMESG("ft6x06_tpd_get_fw_version: version_info=%x \n", version_info);
...
void ft6x06_tpd_get_fw_vendor_name(char * fw_vendor_name)
{
    ...
    TPD_DMESG("ft6x06_tpd_get_fw_vendor_name: tpd_desc=%s \n", tpd_desc);
...
void ft6x06_ftm_force_update(char * ftm_update){
 // if (FACTORY_BOOT == get_boot_mode())
    //{
        ...
        CTP_DBG("ft6x06_ftm_force_update :panel_vendor_buff = %x \n", panel_vendor_buff);
